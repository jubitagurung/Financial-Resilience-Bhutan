...
function toggleChat() {
  ...
  if (isHidden && !chatReady) { initChat(); }
}

function initChat() {
  chatReady = true; chatHistory = [];
  appendChatMsg("bot", "👋 Kuzuzangpola! I'm your <strong>Smart Budget Bhutan</strong> assistant.<br><br>Ask me anything about budgeting, saving, or managing household finances in Bhutan. I'm here to help! 🇧🇹");
}
  ...
  try {
    const res = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        model: "claude-sonnet-4-20250514",
        max_tokens: 1000,
        system: `You are the Smart Budget Bhutan assistant — a friendly, knowledgeable financial advisor specializing in household budgeting and saving for urban Bhutanese families.\n\nKey facts about this website:\n- It analyzes real urban Bhutanese household financial data\n- It promotes the 50/30/20 budgeting rule (50% needs, 30% wants, 20% savings)\n- Income levels covered: Low (Nu. 20,000), Middle (Nu. 40,000), High (Nu. 80,000/month)\n- Currency: Bhutanese Ngultrum (Nu.)\n- It recommends building a 3–6 month emergency fund\n- Key tips: pay yourself first, track spending, avoid lifestyle creep, separate savings account\n\nYour style:\n- Warm, encouraging, and practical\n- Use Ngultrum (Nu.) for all amounts\n- Give specific, actionable advice\n- Keep responses concise and clear (3–5 sentences max unless a detailed breakdown is requested)\n- Occasionally use relevant emojis\n- You can greet in Dzongkha: "Kuzuzangpola" means hello`,
    ...
    appendChatMsg("bot", reply.replace(/\n/g, "<br>"));
  } catch (err) {
    removeTyping(typingId);
...
function appendChatMsg(role, html) {
  ...
  wrap.style.cssText = `display:flex;gap:9px;align-items:flex-end;${role === "user" ? "flex-direction:row-reverse;" : ""}`;
  const avatar = document.createElement("div");
  avatar.style.cssText = "width:28px;height:28px;min-width:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:13px;flex-shrink:0;";
  avatar.style.background = role === "user" ? "rgba(226,95,14,0.25)" : "rgba(201,168,76,0.15)";
  avatar.style.border = role === "user" ? "1.5px solid var(--accent)" : "1.5px solid var(--gold)";
  avatar.textContent = role === "user" ? "👤" : "🪙";
  const bubble = document.createElement("div");
  bubble.style.cssText = `max-width:78%;padding:11px 14px;font-size:13.5px;line-height:1.65;border-radius:${role === "user" ? "14px 14px 4px 14px" : "14px 14px 14px 4px"};`;
  if (role === "user") { bubble.style.background = "rgba(226,95,14,0.18)"; bubble.style.border = "1px solid rgba(226,95,14,0.3)"; bubble.style.color = "var(--text)"; }
  else { bubble.style.background = "rgba(201,168,76,0.08)"; bubble.style.border = "1px solid rgba(201,168,76,0.2)"; bubble.style.color = "var(--text)"; }
...
function appendTyping() {
  ...
  const wrap = document.createElement("div");
  wrap.id = uid;
  wrap.style.cssText = "display:flex;gap:9px;align-items:flex-end;";
  wrap.innerHTML = `<div style="width:28px;height:28px;min-width:28px;border-radius:50%;background:rgba(201,168,76,0.15);border:1.5px solid var(--gold);display:flex;align-items:center;justify-content:center;font-size:13px;">🪙</div><div style="padding:11px 16px;border-radius:14px 14px 14px 4px;background:rgba(201,168,76,0.08);border:1px solid rgba(201,168,76,0.2);display:flex;gap:5px;align-items:center;"><span style="width:7px;height:7px;border-radius:50%;background:var(--gold);animation:bounce 1s infinite;display:inline-block;"></span><span style="width:7px;height:7px;border-radius:50%;background:var(--gold);animation:bounce 1s 0.2s infinite;display:inline-block;"></span><span style="width:7px;height:7px;border-radius:50%;background:var(--gold);animation:bounce 1s 0.4s infinite;display:inline-block;"></span></div>`;
  box.appendChild(wrap);
  box.scrollTop = box.scrollHeight;
  return uid;
}
